...
*/
{
    ...
    #pragma omp parallel for private(j) shared(total, pheromone)
    for ( i = 0 ; i < n ; i++ ) {
        for ( j = 0 ; j < i ; j++ ) {
...
#include <omp.h>
#include <math.h> // Ensure the math library is included for pow()
...
void compute_nn_list_total_information(void)
{
    ...
    #pragma omp parallel for private(j, h) shared(instance, pheromone, total)
    for (i = 0; i < n; i++) {
        for (j = 0; j < nn_ants; j++) {
            h = instance.nn_list[i][j];
            // Ensure updates to pheromone[i][h] are done within a critical section if necessary
            ...
            double tmp = pow(pheromone[i][h], alpha) * pow(HEURISTIC(i,h), beta);
            total[i][h] = tmp;
            total[h][i] = tmp;
        }
    }
...
*/
{
    ...
    #pragma omp parallel for private(j, help_city) shared(pheromone)
    for ( i = 0 ; i < n ; i++ ) {
        for ( j = 0 ; j < nn_ants ; j++ ) {
            help_city = instance.nn_list[i][j];
            pheromone[i][help_city] = (1 - rho) * pheromone[i][help_city];
...
*/
{
    ...
    if ( max_time > 0.1 )
        mutation_strength = 4. * avg_trail * (elapsed_time(REAL) - restart_time) / (max_time - restart_time + 0.0001);
